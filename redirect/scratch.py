...
class name_ban:
    ...
    def help(self):
        print('REDIRECT vers:1')
        about = '''Have you ever wondered: Where does this link go? 
The LINKFINDER it allows you to see the complete path a redirected URL goes through. 
It will show you the full redirection path of URLs, shortened links, or tiny URLs.'''
        ...
        print('\nRun the tool: scratch.py --track')
        print('Commands')
        print("--track or -t  -> Track the given URL's redirection path & find its end URL.")
        print('--help or -h  -> To replay helpline how to use this tool & about tool. ')
        print('\nCoded by: nandhusrn')
        print('nandhusrn')
